# Evidence Inventory | V7 | 2026-08-20
...
| **Assets currently used** | **Zero product images, by Karim's instruction on 2026-08-20: business case study and technical work only, screens and flows to be added later.** The page carries five original SVG diagrams (slot-position model, coverage model, recommendation logic, onboarding funnel, migration continuity) and no screenshots. All 12 PNGs in `assets/money-fellows/` remain on disk and unused. |
...
| **Assets currently used** | All 4 narrative assets (`cover.jpg` hero, `screen-01` landing, `screen-02` loan configuration, `screen-03` active loans) plus one original SVG context diagram, and all 24 UXcel screens as a grouped, captioned public UI library (`#ui-library`). Library composition, verified frame by frame on 2026-08-20: task-led entry (1), loan management (3), public holidays (4), cost centers (3), payroll (3), employee record (1), employee files (3), attendance and approvals (3), worker-facing mobile app (3). Screens 22 to 24 are the worker mobile app, not the web application; the section deck states this and notes that the tablet bezels on web-app frames are presentation frames from the showcase, not the Track B device. |
...
| **Assets currently used** | All 4: `cover.png` (hero plate), `screen-01` (home and discovery), `screen-02` (group-order stepper), `screen-03` (account and product page). Plus one original SVG diagram (group-buying loop). |
...
| **Assets currently used** | The stub is gone. `one-more-thing.html` is now a full case study using 14 of the 22 frames: Inveasy 4, Daajan 4, JumlatyPro 6. Every frame was opened and inspected before captioning. The 8 unused frames are project covers and near-duplicates. The `index.html` tile still uses CSS art, with its decorative line changed from a rhetorical question to the three venture domains. |
